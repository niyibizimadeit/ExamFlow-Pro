      ...
      <main className="animate-fade-in" style={{ padding: "2.5rem 2rem 5rem", maxWidth: "64rem", margin: "0 auto" }}>
        ...
        <header style={{ marginBottom: "2rem" }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
            <div>
              <h1 className="page-heading" style={{ margin: 0 }}>Questions</h1>
              <p className="page-subheading">{total} question{total !== 1 ? "s" : ""} in bank</p>
            </div>
            <div style={{ display: "flex", gap: "0.5rem" }}>
              <Link href="/teacher/questions/categories" className="btn-ghost" style={{ fontSize: "0.75rem" }}>Categories</Link>
        ...
        {/* Filters */}
        <div className="animate-slide-up" style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", marginBottom: "1.25rem" }}>
          <input value={keyword} onChange={e => { setKeyword(e.target.value); setPage(0); }} placeholder="Search questions…" className="input-glass" style={{ width: "12rem" }} />
          <select value={type} onChange={e => { setType(e.target.value); setPage(0); }} className="select-glass" style={{ width: "auto", fontSize: "0.75rem" }}>
            <option value="">All types</option><option value="SINGLE">Single Choice</option><option value="MULTIPLE">Multiple Choice</option><option value="TRUEFALSE">True / False</option><option value="FILL">Fill‑in</option>
          </select>
          <select value={categoryId} onChange={e => { setCategoryId(e.target.value); setPage(0); }} className="select-glass" style={{ width: "auto", fontSize: "0.75rem" }}>
            <option value="">All categories</option>
            {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
          <select value={difficulty} onChange={e => { setDifficulty(e.target.value); setPage(0); }} className="select-glass" style={{ width: "auto", fontSize: "0.75rem" }}>
        ...
        <div className="card animate-slide-up" style={{ overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid rgba(212,180,131,0.2)" }}>
                {["Question", "Type", "Category", "Level", "Score", ""].map((h, i) => (
                  <th key={i} style={{
                    textAlign: i === 5 ? "right" : "left", padding: "0.75rem 1.125rem",
            ...
            <tbody>
              {loading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid rgba(212,180,131,0.1)" }}>
                    {Array.from({ length: 6 }).map((_, j) => (
                      <td key={j} style={{ padding: "0.875rem 1.125rem" }}>
                        <div className="animate-pulse" style={{ height: "0.875rem", borderRadius: "4px", background: j === 0 ? "var(--ink-100)" : "rgba(212,180,131,0.2)", width: j === 5 ? "4rem" : j === 0 ? "80%" : "60%" }} />
                      </td>
                    ))}
                  </tr>
                ))
                ...
                questions.map(q => {
                  ...
                  return (
                    <tr key={q.id} style={{ borderBottom: "1px solid rgba(212,180,131,0.08)", transition: "background 0.12s ease" }}
                      ...
                      <td style={{ padding: "0.875rem 1.125rem", maxWidth: "20rem" }}>
                        <p style={{ fontSize: "0.8125rem", color: "var(--ink-700)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: "'DM Sans', sans-serif" }}>{q.content}</p>
                      </td>
                      <td style={{ padding: "0.875rem 1.125rem" }}>
                        <span style={{
                          fontSize: "0.6875rem", fontWeight: 600, textTransform: "uppercase",
                          letterSpacing: "0.06em", fontFamily: "'DM Sans', sans-serif",
                          padding: "0.15rem 0.5rem", borderRadius: "5px", ...badge,
                        }}>{q.type}</span>
                      </td>
                      <td style={{ padding: "0.875rem 1.125rem", fontSize: "0.8125rem", color: "var(--ink-500)", fontFamily: "'DM Sans', sans-serif" }}>{q.categoryName}</td>
                      <td style={{ padding: "0.875rem 1.125rem", fontSize: "0.8125rem" }}>
                        {q.difficulty === 1 ? "⭐" : q.difficulty === 2 ? "⭐⭐" : "⭐⭐⭐"}
                      </td>
                      <td style={{ padding: "0.875rem 1.125rem", fontSize: "0.8125rem", color: "var(--ink-500)", fontFamily: "'DM Sans', sans-serif" }}>{q.defaultScore} pts</td>
                      <td style={{ padding: "0.875rem 1.125rem", textAlign: "right" }}>
                        <Link href={`/teacher/questions/${q.id}/edit`} style={{ fontSize: "0.75rem", fontWeight: 500, color: "var(--amber-accent)", marginRight: "1rem", textDecoration: "none" }}>Edit</Link>
                        <button onClick={() => del(q.id)} style={{ fontSize: "0.75rem", fontWeight: 500, color: "var(--ink-300)", background: "none", border: "none", cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}
                ...
                <tr><td colSpan={6}>
                  <EmptyState icon={<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
                    title="No questions found" description="Create your first question or adjust the filters." />
                </td></tr>
              )}
            </tbody>
          </table>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: "0.375rem", marginTop: "1.25rem" }}>
            {Array.from({ length: totalPages }, (_, i) => (
              <button key={i} onClick={() => setPage(i)}
                style={{
                  minWidth: "2.25rem", height: "2.25rem", borderRadius: "9px",
